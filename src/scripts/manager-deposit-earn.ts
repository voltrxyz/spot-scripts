...
import { depositStrategyAmount } from "../../config/spot";
...
const main = async () => {
  ...
  const depositStrategyIx = await getDepositStrategyInstructionAsync({
    ...
    instructionDiscriminator: new Uint8Array(DISCRIMINATOR.DEPOSIT_JUPITER_EARN),
    additionalArgs: null,
  });
  ...
  console.log("Jupiter earn deposited with signature:", txSig);
};
